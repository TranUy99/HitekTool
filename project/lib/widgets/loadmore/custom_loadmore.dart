...
import 'package:project/widgets/custom_no_result_widget.dart';
import 'package:project/widgets/loadmore/custom_load_more_delegate.dart';
...
class LoadMoreState extends State<LoadMore> {
  ...
  LoadMoreDelegate get loadMoreDelegate => const CustomLoadMoreDelegate();
  ...
  _renderCustomNoResultWidget() {
    if (emptyWidget == null) {
      return const CustomNoResultWidget();
    }
    return emptyWidget!;
  }
  ...
  final ValueNotifier<LoadMoreStatus> _loadMoreStatus = ValueNotifier(LoadMoreStatus.idle);
  ...
  Widget? _buildGridView(GridView gridView) {
    ...
    if (delegate is SliverChildBuilderDelegate) {
      ...
      if (viewCount == 0) return _renderCustomNoResultWidget();
      ...
      return CustomScrollView(
        ...
        slivers: <Widget>[
          ...
          SliverToBoxAdapter(
            child: isEnd ? _buildLoadMoreView() : const SizedBox(),
          ),
        ],
      );
    }
  ...
  Widget? _buildListView(ListView listView) {
    ...
    if (delegate is SliverChildBuilderDelegate) {
      ...
      if (viewCount == 1) return _renderCustomNoResultWidget();
